...
public class AbstractSqlParser implements SelectVisitor, FromItemVisitor, SelectItemVisitor, ExpressionVisitor, OrderByVisitor, ItemsListVisitor {
	@Override
    ...
    public void visit(Function fnctn) {
        if (fnctn.getParameters() != null) {
        	fnctn.getParameters().getExpressions().forEach(e -> {
        		e.accept(this);
        	});
        }
    }
    ...
    public void visit(CaseExpression ce) {
    	List<WhenClause> wcs = ce.getWhenClauses();
    	if (null != wcs) {
    		wcs.forEach(c -> {
    			c.accept(this);
    		});
	...
	@Override
	public void visit(BitwiseRightShift exp) {
		 exp.getLeftExpression().accept(this);
		 exp.getRightExpression().accept(this);		
	}

	@Override
	public void visit(BitwiseLeftShift exp) {
		 exp.getLeftExpression().accept(this);
		 exp.getRightExpression().accept(this);		
	}
